...
foreach($empRecords as $row){
   $data[] = array(
      ...
      "action"=>'
          ...
          <a href="index.php?page=member_edit&&edit_id='.$row['id'].'" class="dropdown-item" type="button" id="memberEdit_btn">Edit customer</a>
            
            <a href="index.php?page=sell_pay&&id='.$row['id'].'" class="dropdown-item" type="button">Pay now</a>

             <p id="memberDelete_btn" style="padding-left:15px;cursor:pointer;color:red;" data-id="'.$row['id'].'">Delete</p>
          </div>
        </div>
      ',
   );
